...
class HotSalesViewPagerAdapter(
    ...
    override fun onBindViewHolder(holder: HotSaleViewHolder, position: Int) {
        ...
        with(holder.binding) {
            ...
            tvTitleHotSale.text = item.title
            tvSubtitleHotSale.text = item.subtitle
            flFullInfoItem.visibility = if (item.isNew) View.VISIBLE else View.INVISIBLE

            Glide.with(ivHotSale).load(item.picture).override(600, 400)
                .into(ivHotSale)
        }
    }
